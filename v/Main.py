...
# from PredictiveManager import PredictiveManager  # Disabled for demo
...
def ensure_evdev_map(force=False, moza_event=None, arduino_event=None):
    ...
    if os.path.exists(cfg_file) and not force:
        print(f"âœ… [evdev] mapping present: {cfg_file}")
        return True

    os.makedirs(cfg_dir, exist_ok=True)
    wizard = os.path.join(root, "Tools", "evdev_autotest.py")
    if not os.path.exists(wizard):
        logging.warning("âš ï¸evdev_autotest.py not found at Tools/; skipping auto-capture.")
        return False
    ...
    print("\n>>> Running input capture (evdev_autotest). "
          "Follow prompts for LEFT/RIGHT/PUSH/PULL and seatbeltâ€¦")
    try:
        rc = subprocess.call(cmd)
        if rc != 0:
            logging.warning(f"âš ï¸evdev_autotest exited with code {rc}")
    except Exception as e:
        logging.exception(f"âš ï¸Failed to run evdev_autotest: {e}")

    if not os.path.exists(cfg_file):
        logging.warning("âš ï¸input_devices.json was not created. "
                        "Run Tools/evdev_autotest.py manually if needed.")
        return False

    print(f" âœ…ðŸ•¹ï¸ Captured device mapping â†’ {cfg_file}")
    return True
...
def game_loop(args, client, monitors, joystick_mappings=None):
    ...
    """
#    logging.info("âœ…GAME LOOP: Initializing new session.")
    # Yeah,
...
#
        ...
        print(f"ðŸŽ¯ Using {layout_strategy} layout mode ({total_logical} logical displays)")
        ...
        if args.layout_mode == "single":
            ...
            total_height = single_monitor_height
            logging.info(f" âœ…ðŸ–¥ï¸ Layout: {args.layout_mode}  Window={total_width}x{total_height}")
            iLib.ilog("info", f"Layout: {args.layout_mode}  Window={total_width}x{total_height}", "status_alerts","debug",1 )
        else:
            total_width = single_monitor_width*4
            total_height = single_monitor_height
            logging.info(f" âœ…ðŸ–¥ï¸ðŸ–¥ï¸ðŸ–¥ï¸ðŸ–¥ï¸ Layout: {args.layout_mode}  Window={total_width}x{total_height}")
...
#        logging.info(f"ðŸ–¥ï¸ðŸ–¥ï¸ðŸ–¥ï¸ðŸ–¥ï¸ðŸ–¥ï¸ðŸ–¥ï¸xrandr output: {xrandr_output}")

        if args.dev:
            from VehicleLibrary import VehicleLibrary
            
            logging.info("ðŸš€ Developer mode: skipping selection screen, using defaults")
        ...
        if args.xodr_path:
            if os.path.exists(args.xodr_path):
                with open(args.xodr_path, encoding='utf-8') as od_file:
                    data = od_file.read()
                logging.info(f" ðŸ—ºï¸Loading map from OpenDRIVE file: {os.path.basename(args.xodr_path)}")
            ...
            if chosen_map_id:
                logging.info(f" ðŸ—ºï¸Loading chosen map: {chosen_map_id}")
                carla_world = client.load_world(chosen_map_id)
            else:
                logging.info(f"â“No chosen map returned, loading default from cmdline: {args.map}")
        ...
        if args.sync:
            ...
            logging.info("â±ï¸ SYNCHRONOUS MODE applied successfully")
        ...
        if world_obj.player and args.max_rpm > 0:
            ...
            logging.info(f"âš–ï¸ Applied custom max_rpm of {args.max_rpm} to vehicle.")
        ...
        if args.sync:
            ...
            for _ in range(15):
                carla_world.tick()
            logging.info("âœ…â±ï¸ Stabilization complete.")

        data_ingestor = DataIngestion.DataIngestion()
        # predictive_manager = PredictiveManager(data_ingestor)  # Disabled for demo
        ...
        while True:
            ...
            if not world_snapshot:
                logging.error("âŒFailed to get world snapshot.")
                continue

            if controller.parse_events(world_obj, clock):
                logging.info("ðŸUser requested exit.")
                break

            if mvd_feature_extractor._catastrophic_failure_occurred:
                logging.warning("âŒCatastrophic failure detected. Ending session.")
                break
            ...
            if world_obj.player and world_obj.player.is_alive:
                ...
                controller._seatbelt_state = True
                ...
                # Predictive manager disabled for demo
                ...
                # Update HUD with MVD scores
                mbi = standardized_indices.get('MBI', 0.0)
                lmi = standardized_indices.get('LMI', 0.0)
                hud.update_mvd_scores_for_display(overall_dp_score, mbi, lmi)
        ...
        # --- End Screen Logic ---
        logging.info("ðŸ Session ended. Presenting end screen.")
        ...
        logging.critical(f"â˜ ï¸ Critical error in game loop: {e}", exc_info=True)
...
#            hardware_bridge.stop()
        ...
        logging.info("ðŸ GAME LOOP: Session cleanup complete.")
...
def main():
    ...
    if not original_layout:
        print("âš ï¸ Could not detect monitor setup. Exiting.")
        return
...
# --- Pre-Welcome quick setup ---
        ...
        if not args.no_launch_carla:
            if not args.carla_root:
                logging.error("â“ CARLA_ROOT path not provided.")
                sys.exit(1)
...
#                logging.info(f"Successfully connected to CARLA Server {client.get_server_version()}")
                ...
                logging.warning(f"âš ï¸Connection failed: {e}. Retrying...")
                if i == max_retries - 1:
                    raise
        ...
        logging.critical(f"â˜ ï¸Unhandled exception in main: {e}", exc_info=True)
    finally:
        ...
        pygame.quit()
        logging.info("ðŸâœ…Main script execution finished.")
