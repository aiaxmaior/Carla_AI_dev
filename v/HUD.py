...
class HUD(object):
    def __init__(self, width, height, args):
        ...
        logging.info(f"Screen-Vehicle Scale Factor{scale_factor}")
        iLib.ilog("info", f"Screen-Vehicle Scale Factor: {scale_factor}", "sysUI","i")
        """
        ...
        """
        self.panel_fonts = fLib.get_loaded_fonts(font="tt-supermolot-neue-trl.bd-it", type="panel_fonts", scale=scale_factor)
    ...
    def tick(self, world_instance, clock, idling , controller, display_fps):
        ...
        if self._show_info and world_instance.player and world_instance.player.is_alive:
            ...
            self._sub_label_values = {
                "Steering Management Score":f"{self.mbi_display:.2f}",
                "Lane Management Score":f"{self.lmi_display:.2f}",                
            }
...
#                    self.hud._render_blinker_indicator(display)
                    logging.info(f'rearview {x_pos}y,{y_pos}')
#        self.hud._render_blinker_indicator(display)
