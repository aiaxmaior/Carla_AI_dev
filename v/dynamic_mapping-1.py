...
from FontIconLibrary import FontLibrary, IconLibrary
...
class DynamicMapping:
    ...
    def __init__(
        ...
        self.JOYSTICK_MAPPING_FILE = (
            "./configs/joystick_mappings/joystick_mappings.json"
        )
    ...
    def run_configuration(self):
        ...
        try:
            loaded_mappings = self.check_for_saved_mappings(
                display_surface, primary_font
            )
        except Exception as e:
    ...
    def _display_mapping_message(
        ...
        title_surf = title_font.render(
            "CONTROLLER CONFIGURATION", True, (169, 204, 227)
        )
        surface.blit(
            title_surf,
            title_surf.get_rect(center=(center_x, surface.get_height() // 6)),
        )

        text_surface = main_font.render(primary_message, True, (189, 195, 199))
        surface.blit(
            text_surface,
            text_surface.get_rect(center=(center_x, surface.get_height() // 3)),
        )
        ...
        if instruction_message:
            ...
            surface.blit(
                instr_surf,
                instr_surf.get_rect(center=(center_x, surface.get_height() // 2)),
            )
        ...
        if sub_message:
            ...
            for line in lines:
                ...
                rect = sub_surf.get_rect(
                    center=(center_x, surface.get_height() * 2 // 3 + y_offset)
                )
        ...
        if detected_value_str:
            ...
            surface.blit(
                det_surf,
                det_surf.get_rect(center=(center_x, surface.get_height() * 3 // 4)),
            )
            ...
            for event in pygame.event.get():
                ...
                # --- Handle mapped joystick UI controls (Enter/Escape/Restart) for button/axis/hat ---
                    ...
                    if self._check_mapped_button(event, "UI_RESTART"):
                        return "RESTART_CONFIG"
                    if self._check_mapped_button(event, "Enter"):
                        if input_type_expected == "axis_steer" and axis_steer_data.get(
                            "moved_significantly"
                        ):
                            current_joy_idx = axis_steer_data.get("joy_idx")
                            current_axis_idx = axis_steer_data.get("axis_idx")
                            if (
                                isinstance(current_joy_idx, int)
                                and isinstance(current_axis_idx, int)
                                and 0 <= current_joy_idx < len(self._joysticks)
                                and self._joysticks[current_joy_idx]
                            ):
                                try:
                                    center_val = self._joysticks[
                                        current_joy_idx
                                    ].get_axis(current_axis_idx)
                                except pygame.error:
                                    center_val = 0.0
                                ...
                                return {
                                    ...
                                    "joy_idx": current_joy_idx,
                                    "id": current_axis_idx,
                                    "raw_calibrated_min": float(
                                        axis_steer_data.get("min", 0.0)
                                    ),
                                    "raw_calibrated_max": float(
                                        axis_steer_data.get("max", 0.0)
                                    ),
                                    "raw_calibrated_center": float(
                                        axis_steer_data.get("center", 0.0)
                                    ),
                                }
                    if self._check_mapped_button(event, "Escape"):
                        return None
                    ...
                    # --- Handle Mapped Joystick UI Buttons ---
                        ...
                        enter_joy_id = enter_map.get("joy_id")
                        enter_btn_id = enter_map.get("button_id")

                        # Only proceed if this JOYBUTTONDOWN matches the configured Enter button
                        if (
                            enter_joy_id is not None
                            and enter_btn_id is not None
                            and event.instance_id == enter_joy_id
                            and event.button == enter_btn_id
                        ):
                            # Confirm steer axis only when we've seen meaningful motion
                            if (
                                input_type_expected == "axis_steer"
                                and axis_steer_data.get("moved_significantly")
                            ):
                                current_joy_idx = axis_steer_data.get("joy_idx")
                                current_axis_idx = axis_steer_data.get("axis_idx")
    ...
    def _check_mapped_button(self, event, mapping_name):
        ...
        """
        mapping = (
            self._map_keys.get(mapping_name, {})
            if mapping_name in ["Enter", "Escape"]
            else self.mapped_controls.get(mapping_name, {})
        ) or {}
        mtype = mapping.get("type", "button")
        event_joy_id = getattr(event, "instance_id", getattr(event, "joy", None))
        joy_id = mapping.get("joy_id")
        if joy_id is not None and event_joy_id != joy_id:
            return False

        if mtype == "button":
            return event.type == pygame.JOYBUTTONDOWN and event.button == mapping.get(
                "button_id"
            )
        elif mtype == "axis":
            if event.type != pygame.JOYAXISMOTION:
            ...
            return (
                tuple(target) == tuple(getattr(event, "value", (0, 0)))
    ...
    def _configure_controls_dynamically(self, surface, font):
        ...
        actions_to_map = [
            ...
            {"id": "BLINKER_LEFT", "type": "button", "prompt": "Left Blinker"},
            {
                "id": "BLINKER_LEFT_OFF",
                "type": "button",
                "prompt": "Reset Blinker to Middle",
            },
            {"id": "BLINKER_RIGHT", "type": "button", "prompt": "Right Blinker"},
            {
                "id": "BLINKER_RIGHT_OFF",
                "type": "button",
                "prompt": "Reset Blinker to Middle",
            },
            ...
            {
                "id": "NEXT_WEATHER_REVERSE",
                "type": "button",
                "prompt": "Previous Weather Setting",
            },
        ]
        self._display_mapping_message(
            surface,
